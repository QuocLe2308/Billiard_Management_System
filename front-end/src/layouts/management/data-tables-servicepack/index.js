...
function DataTables() {
  ...
  const [dataTableData, setDataTableData] = useState({
    columns: [
      ...
      { Header: "Detail", accessor: "detail", width: "7%" },
    ...
    try {
      ...
      getRequest("/api/servicePack", (response) => {
        if (response.status === "success") {
          ...
          const formattedData = serviceData.map((serviceData) => ({
            id: serviceData.servicePackId,
            name: serviceData.namePack,
            price: serviceData.money,
            detail: serviceData.serviceDetail,
            actionCell: (
